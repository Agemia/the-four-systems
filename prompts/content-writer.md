...
Optional: `context/editorial-charter.md` (generated by the `setup` skill from the user's good/bad article examples). **If present, read it on every run.** Its rules are binding:
- The "What a good article looks like here" patterns guide the Step 3 outline and Step 4 draft.
- The "Anti-patterns" section is treated as review failures: any anti-pattern present in the draft fails the Step 5 checklist exactly like a banned word would.
- The "Self-review rubric additions" checkboxes are appended verbatim to the Step 5 checklist.
- The "Style DNA" section complements `tone-of-voice.md`; when the two conflict, `tone-of-voice.md` wins (the user wrote it explicitly, the charter was inferred from examples).
