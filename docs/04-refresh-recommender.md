...
The output is a prioritised refresh queue with a specific action per URL. You execute the actions yourself (or via your CMS / refresh tool of choice). System 4 is a recommender, never an auto-rewriter.
...
For content refreshes, Layer 2 also runs a local SERP comparison with DataForSEO. It recovers the target keyword, queries Google organic results for the configured market (France / French by default), analyses the top 5 organic results, and records the dominant intent plus the concrete content gaps between the current page and what is ranking.
...
- DataForSEO SERP analysis: only for URLs that need a content refresh, one local organic SERP call per target keyword plus page inspection where available
- Claude Layer 2 classification: ~$0.05 per run

Weekly with default 60-URL cap stays cheap because pure indexing and canonical fixes skip SERP analysis. Cost scales with the number of stale pages that actually need refresh briefs.
...
- **Layer 2 table** (the one you act on): URL | Keyword | SERP intent | Action | Priority | Primary flag | Age | Content gaps | Recommendation | Status
...
For content refreshes, the recommendation also includes the local SERP intent and the highest-impact gaps. Example: if the French top 5 all include a comparison table, pricing examples, and FAQ schema, but your article is only a definition-style post, the queue item should tell you to shift the page toward that intent and add those missing elements.
