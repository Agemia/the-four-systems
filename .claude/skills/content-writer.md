---
name: content-writer
description: Write a blog post for the user's site interactively, following the 5-step workflow (brief, research, outline, draft, review). Pulls the next item from the System 1 content queue, walks the user through each decision, drafts a post that meets the 2026 GEO / AI Search bar: Three Kings keyword placement, Content Capsule sections with passage self-containment (citable in isolation), one mandatory Information Gain section (unique value vs the current SERP), salient-entity coverage, unbroken heading hierarchy, inline 1-3 word citations, locale-aware SERP research. Then ticks the queue item off. Use when the user says "write a post", "draft the next one", "let's write something", "content writer", or asks for a blog post on any topic.
