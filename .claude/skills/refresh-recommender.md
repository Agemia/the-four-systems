...
description: Run on-demand GSC health scan for the user's site. Pulls 28-day vs 28-day GSC data, applies intent-sensitive staleness thresholds (news 90d, YMYL 180d, software 270d, money pages 365d, evergreen 18mo, reference 24mo), flags decaying pages, pages stuck at positions 5-15, pages dropped out of the top 10, CTR outliers, and AI Overview citation losses (URLs that were AIO-cited last snapshot but are no longer). Classifies each alert as request_indexing / fix_canonical / refresh / audit_then_decide, then runs a locale-aware SERP comparison (locale from context/site-config.md) including AI Overview citations and SERP feature presence for refresh briefs. Use when the user asks for an SEO health check, vital signs, decay scan, GSC audit, AI Overview tracking, or "what's broken on the site".
allowed-tools: Read, Write, Edit, Bash, WebFetch, mcp__dfs-mcp__serp_organic_live_advanced, mcp__dfs-mcp__on_page_content_parsing
---
